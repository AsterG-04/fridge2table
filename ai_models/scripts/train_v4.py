...
ingredient dataset (ai_models/dataset_v4 -- the 56 v3 classes plus 12 new
fruit/vegetable classes), and saves:

    ai_models/model/ingredient_classifier_v4.keras
    ai_models/model/class_names_v4.json
...
Does NOT overwrite the v3 model files -- this is a separate, comparable
output so v3 stays as a safe fallback until v4 is verified to be as good
or better.

Run from ai_models/ with the venv activated:
    venv\\Scripts\\python.exe scripts\\train_v4.py
"""
...
TRAIN_DIR = os.path.join(BASE_DIR, "dataset_v4", "train")
TEST_DIR = os.path.join(BASE_DIR, "dataset_v4", "test")
...
def main():
    ...
    model_path = os.path.join(MODEL_DIR, "ingredient_classifier_v4.keras")
    model.save(model_path)
    print(f"Saved model to {model_path}")

    class_names_path = os.path.join(MODEL_DIR, "class_names_v4.json")
