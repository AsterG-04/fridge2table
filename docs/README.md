...
| [API_REFERENCE.md](API_REFERENCE.md) | Every backend endpoint: method, URL, params, request/response schema, worked examples, and an explicit note on the current lack of server-side auth verification. |
...
| [AUDIT_FIXES.md](AUDIT_FIXES.md) | What changed after the documentation audit above surfaced real gaps (disconnected notifications, dead code, misleading Privacy Policy/Cloud Sync wording, missing server-side auth, UI-only placeholders) — what was fixed, what was deliberately left alone, and the one manual Supabase/Render setup step still required. Read this alongside the docs above, which still describe pre-fix behavior in places. |
